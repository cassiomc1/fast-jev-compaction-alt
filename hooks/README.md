...
Responses are validated before the hook uses them. The shared library accepts
generic `choice` and `score` questions for routing and triage, while this hook
uses `noul` probabilities for keep/drop decisions. A Jev response is evidence,
not authorization: autonomous actions must apply a separate local policy and
approval boundary.
...
The repository also includes programming-focused regression scenarios covering
read/grep/edit/test flows, exact test failures, large coding histories, and
bounded concurrent Jev batches. Run them from the repository root with:
